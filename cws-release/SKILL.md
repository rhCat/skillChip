...
perks: [sign, transparency, engine]
---
...
The cores are `infra/cwp/{release,translog,engineattest}.py`; each perk's hermetic self-test signs with an
ephemeral key so it runs in CI (needs only openssl/ed25519ph), while a real release uses the committed pinned
root + the publisher's offline key.

## Coming (the rest of the M4 cone)
`publish` (P3-T15, dual-signed release receipts via the channel). The Citrinitas publish gate (P3-T09)
depends on the `alchemy` validator (concordance, P3-T08), which is not built yet.

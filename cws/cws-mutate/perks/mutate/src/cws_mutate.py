#!/usr/bin/env python3
"""cws_mutate — mutation-test a gate: mutate its source, run its test slice, score the survivors.
...
def main() -> int:
    ...
    try:
        ...
        if not os.path.isfile(tpath):
            ...
            print(json.dumps({"tool": "cws_mutate", "status": "error", "reason": "target_missing", "report": out}))
            return 1
        ...
        if baseline != 0:
            report = {"target": target, "error": f"baseline test failed (exit {baseline}) — score undefined",
                      "mutants": 0, "killed": 0, "survived": [], "mutation_score": 0.0}
            json.dump(report, open(out, "w"), indent=2)
            print(json.dumps({"tool": "cws_mutate", "status": "error", "reason": "baseline_failed", "report": out}))
            return 1
        ...
        print(json.dumps({"tool": "cws_mutate", "status": "ok" if ok else "weak", "mutants": total,

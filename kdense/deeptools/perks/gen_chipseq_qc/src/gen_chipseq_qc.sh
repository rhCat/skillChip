#!/usr/bin/env bash
# gen_chipseq_qc — generate a ChIP-seq QC workflow bash script via vendored workflow_generator.py (stdlib).
# Read-only: emits a template script, runs nothing. Emits one structured-JSON audit line.
set -uo pipefail
: "${RECORD_STORE:?}"
: "${INPUT_BAM:=}" "${CHIP_BAMS:=}" "${GENOME_SIZE:=}" "${THREADS:=}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUT="${RECORD_STORE%/}/chipseq_qc.sh"
...
ARGS=(chipseq_qc -o "$OUT")
[ -n "$INPUT_BAM" ]   && ARGS+=(--input-bam "$INPUT_BAM")
[ -n "$CHIP_BAMS" ]   && ARGS+=(--chip-bams "$CHIP_BAMS")
...
[ -s "$OUT" ] || printf '{}' > "$OUT"
printf '{"tool":"gen_chipseq_qc","status":"ok","workflow":"%s"}\n' "$OUT"

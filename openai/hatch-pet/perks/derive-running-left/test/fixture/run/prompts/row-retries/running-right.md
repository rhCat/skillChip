Create Codex pet row `running-right` for `sprout`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A compact original Codex pet ready for animation. Preserve silhouette, face, palette, material, proportions, markings, and props.

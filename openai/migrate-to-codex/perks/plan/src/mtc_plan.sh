#!/usr/bin/env bash
# mtc_plan — print the staged Codex migration plan: artifact paths + report rows (--plan, read-only, no writes).
...
# --plan stages artifacts and prints the plan; it never writes target files. || true keeps the porter green.
PYTHONPATH="$HERE" "$PY" "$HERE/migrate-to-codex.py" --source "$SOURCE" --target "$TARGET" --plan >> "$OUT" 2>&1 || true
[ -s "$OUT" ] || printf '{}' > "$OUT"
printf '{"tool":"mtc_plan","status":"ok","source":"%s","target":"%s","plan":"%s"}\n' "$SOURCE" "$TARGET" "$OUT"

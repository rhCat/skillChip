#!/usr/bin/env bash
# pw_network — dump the page's network activity via the bundled wrapper (read-only).
# Structured JSON output (audit/debug log). Degrades gracefully when a live browser is not available.
set -uo pipefail
: "${RECORD_STORE:?}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUT="${RECORD_STORE%/}/network.json"
...
if [ -n "${PW_LIVE:-}" ] && command -v npx >/dev/null 2>&1; then
  bash "$WRAP" network >> "$OUT" 2>&1 || true
else
  printf '{"tool":"pw_network","status":"degraded","reason":"live browser run not requested (set PW_LIVE=1 with npx present)","planned":"playwright-cli network"}\n' >> "$OUT"
fi

[ -s "$OUT" ] || printf '{}' > "$OUT"
printf '{"tool":"pw_network","status":"ok","out":"%s"}\n' "$OUT"
